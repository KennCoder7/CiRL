...
class Processor(IO):
        ...
        if self.arg.phase == 'train':
            self.data_loader['train'] = torch.utils.data.DataLoader(
                ...
                num_workers=self.arg.num_worker * torchlight.ngpu(
                    self.arg.device),
                drop_last=True)
            self.meta_info['iter_per_epoch'] = len(self.data_loader['train'])
    ...
    def start(self):
        ...
        # test phase
            ...
            # evaluation
            self.io.print_log('Visualize Start:')
            self.visualize()
            self.io.print_log('Done.\n')

        elif self.arg.phase == 'save_att_map':
            # the path of weights must be appointed
            if self.arg.weights is None:
                print('Please appoint --weights.')
            self.io.print_log('Model:   {}.'.format(self.arg.model))
            ...
            # evaluation
            self.io.print_log('Save Attention Map Start:')
            self.save_att_map()
            self.io.print_log('Done.\n')
