...
def uniform_sample(data_numpy, target_frame_num):
    C, T, V, M = data_numpy.shape
    data_sampled = np.zeros((C, target_frame_num, V, M))
    for v in range(V):
        for m in range(M):
            data_sampled[:, :, v, m] = uniform_sample_single(data_numpy[:, :, v, m], target_frame_num)
    return data_sampled

def uniform_sample_single(data_numpy, target_frame_num):
    C, T = data_numpy.shape
    data_sampled = np.zeros((C, target_frame_num))
    if T <= target_frame_num:
        data_sampled[:, :T] = data_numpy
    else:
        interval = T / target_frame_num
        for i in range(target_frame_num):
            data_sampled[:, i] = data_numpy[:, int(i * interval)]
    return data_sampled